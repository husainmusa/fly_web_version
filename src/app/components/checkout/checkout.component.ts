...
SetCrruntLocation(){
    ...
    new google.maps.Marker({
      ...
      geocoder.geocode({'location': latlng}, function(results) {
        ...
        if (results[0]) {
          ...
          me.zone.run(()=>{
            console.log()
            if(me.responseAddress){
              me.address =  me.responseAddress[1]
              me.house =  me.responseAddress[0][1].long_name
              me.landmark =  me.responseAddress[0][2].long_name
            }
  ...
  async createOrder(payMethod, payKey) {
    ...
    this.api.post('orders/save', param).subscribe((data: any) => {
      console.log(data,'orders/save store_id ::',data.data.store_id);
  ...
  submit() {
    ...
    if (this.payMethods === 'cod') {
      ...
      localStorage.setItem('deliveryAddress', JSON.stringify(this.cart.deliveryAddress));
      localStorage.setItem('datetime', this.datetime);
      const url = 'https://secure.e-c.co.il/easycard/createform.asp?RedirectToken=ecee3392-2939-4a46-b40e-c5b080b44fb5&Sum='+this.cart.totalPrice+'&MType='+this.currencyValue+'&returnURLTrue=https://flyvip.biz/success&returnURLFalse=https://flyvip.biz/success';
